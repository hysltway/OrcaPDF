...
**OrcaPDF** 是一款免费开源的 **双栏 PDF 实时对照翻译工作台**，旨在提供舒适的学术文献阅读与翻译体验。本项目致力于打造一个本地化、高性能的学术翻译方案，替代“小绿鲸”等商业 PDF 翻译工具。（小绿鲸每月三次全文翻译额度/沉浸式翻译的pdf翻译也是拉完了）

通过集成 **硅基流动API**（你也可以用Google Cloud Translation API） 与智能排版算法，OrcaPDF 能够在翻译文本的同时，**最大程度保留原始 PDF 的页面结构、多栏排版、数学公式、高精图表与图注位置**，让学术阅读流畅无阻。
